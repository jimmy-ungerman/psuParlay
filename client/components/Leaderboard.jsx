...
export default function Leaderboard() {
  ...
  if (loading) return <div className="p-6 text-center text-gray-500">Loading standings...</div>;
  if (error)   return <div className="p-6 text-center text-red-400">{error}</div>;

  return (
    <div className="p-4 space-y-4">
      <div className="flex items-center justify-between">
        <h2 className="text-sm font-semibold text-gray-400 uppercase tracking-wide">{season} Standings</h2>
      </div>
        ...
        <div className="space-y-2">
          {data.map((entry, idx) => {
            ...
            return (
              <button
                key={entry.id}
                onClick={() => !isMe && setH2hUserId(entry.id)}
                disabled={isMe}
