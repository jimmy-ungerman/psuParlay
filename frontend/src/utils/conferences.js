// Maps ESPN display team names to conferences.
// Matching is done by checking if the game's team name includes the key.
...
export function getConference(teamName) {
  ...
  for (const [key, conf] of Object.entries(TEAMS)) {
    if (teamName.includes(key)) return conf;
  }
  return 'Other';
}

// Returns the conference for a game (uses home team's conference)
export function getGameConference(game) {
  return getConference(game.home_team);
}
